...
namespace DataCurveSeer.Config
{
    public class ConfigGeneral : PageBuilderAndMenu.clsPageBuilder, IConfigPage
    {
        ...
        private void HandleDaysOfDataStorageChange(Dictionary<string, string> dicQueryString)
        {
            ...
            var numberOfDaysAsString = HttpUtility.UrlDecode(dicQueryString[DaysOfDataStorageKey]);
            int numberOfDays = 0;
            if (int.TryParse(numberOfDaysAsString, out numberOfDays))
            {
                if (numberOfDays > 1)
                {
                    _iniSettings.DaysOfDataStorage = numberOfDays;
                }
                return;
            ...
            PostError("The number of days to keep data must be a positive value bigger than 1");
