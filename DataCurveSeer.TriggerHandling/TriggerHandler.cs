...
namespace DataCurveSeer.TriggerHandling
{
    ...
    public class TriggerHandler : ITriggerHandler, IDisposable
    {
        ...
        private void CreateExistingTriggers(List<IPlugInAPI.strTrigActInfo> triggersInPlugin)
        {
        ...
        private void WaitAndRefetchTriggers()
        {
            if (_triggersFromHomeSeerHandler == null)
            {
                ...
                _triggersFromHomeSeerHandler.TriggerDataReady += HomeSeerTriggerDataReady;
                _triggersFromHomeSeerHandler.StartWork();

            }
            else if (_triggersFromHomeSeerHandler.IsRunning)
            {
                _triggersFromHomeSeerHandler.DelayFetching(20);
            }
            else
            {
                _triggersFromHomeSeerHandler.StartWork();
        ...
        private void HomeSeerTriggerDataReady(object sender, TriggersInHomeSeerDataEventArgs eventArgs)
        {
            if (_watchedEventDeviceIds.Count > 0)
            {
                _watchedEventDeviceIds.Clear();
            }
            if (eventArgs.TriggersInPlugin != null & eventArgs.TriggersInPlugin.Count> 0)
            {
                CreateExistingTriggers(eventArgs.TriggersInPlugin);
            }
        }
